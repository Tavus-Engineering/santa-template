	...
	return (
		...
		>
			<div className={styles.buttonJoinInner}>
				...
				/>
				<span>{loading ? 'Dialing in to the North Pole...' : 'JOIN VIDEO CALL'}</span>
			</div>
		</button>
	);
};
...
export const HairCheck = memo(({ isJoinBtnLoading = false, onJoin, onCancel }) => {
	...
	// Use camera ready state OR permissions granted for button enable
	const canProceed = isPermissionsGranted || (isCamReady && cameraStarted);
